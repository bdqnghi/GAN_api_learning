...
def get_candidates_2(emb1, emb2, params, build="S"):
    ...
    # print("Candidates emb : " + str(candidate_embs.size()))
    scores = emb2.mm(candidate_embs.transpose(0, 1)).transpose(0, 1)
    ...
    print("All score size : " + str(all_scores.size()))
    print("All targets size : " + str(all_targets.size()))
    # print(all_targets)
    all_pairs = torch.cat([
...
def build_dictionary(src_emb, tgt_emb, mutual_nn, params, s2t_candidates=None, t2s_candidates=None):
    ...
    if params.dico_build == 'S2T':
        ...
        if mutual_nn == 1:
            ...
            # s2t_candidates_sorted = sorted(s2t_candidates, key=lambda tup: tup[0])
            # t2s_candidates_sorted = sorted(t2s_candidates, key=lambda tup: tup[0])
